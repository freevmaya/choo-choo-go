class EndTrack extends StraightTrack {
	...
	createModel() {
        ...
        const leftStay = this.createBox(width, height, width, this.sleeperMaterial);
        leftStay.position.set(-railSpacing / 2, h2, center);
        group.add(leftStay);
        
        const rightStay = this.createBox(width, height, width, this.sleeperMaterial);
        rightStay.position.set(railSpacing / 2, h2, center);
        group.add(rightStay);
        
        const box = this.createBox(this.getConst('SLEEPER_LENGTH'), width, width, this.sleeperMaterial);
